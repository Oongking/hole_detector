...
mono_img = np.ones([50,50])
cv2.imshow("mono_img",resize_percent(mono_img,50))
cv2.createTrackbar("threshold","mono_img", 0,255,nothing)
...
while not rospy.is_shutdown():
    ...
    if waitkeyboard & 0xFF==ord('s'):
        cv2.imwrite(f"{folder_path}/{i}.png",mono_img)
        i+=1
    ...
    if waitkeyboard & 0xFF==ord('h'):
        while not rospy.is_shutdown():
            ...
            mono_img = cam.get_img()
            waitkeyboard = cv2.waitKey(1)
            th_val = cv2.getTrackbarPos("threshold","mono_img")
            ret,thresh1 = cv2.threshold(mono_img,th_val,255,cv2.THRESH_BINARY)


            cv2.imshow("mono_img",resize_percent(thresh1,50))


            if waitkeyboard & 0xFF==ord('e'):
                print(f"last threshold : {th_val}")
                break
